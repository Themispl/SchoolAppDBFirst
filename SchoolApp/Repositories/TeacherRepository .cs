...
namespace SchoolApp.Repositories
{
    ...
    {
        public TeacherRepository(Student6DbContext context) : base(context)
        {
        }
        ...
        public async Task<Teacher?> GetByPhoneNumberAsync(string phoneNumner)
        {
            return await context.Teachers.Where(t => t.PhoneNumber == phoneNumner).FirstOrDefaultAsync();
        }
        ...
        {
            List<Course> courses;
            courses =  await context.Teachers.Where(t => t.Id == id).SelectMany(t => t.Courses).ToListAsync();
            return courses;
        }
